...
export default function LoginPage() {
  ...
  const onGoogleSuccess = async (cred: CredentialResponse) => {
    try {
      ...
      toast.success('Signed in with Google', { description: `Hi ${data?.user?.name || ''}` })
      router.push(redirect)
    } catch (err: any) {
      ...
      <Card className="w-full max-w-md shadow-lg rounded-2xl border border-slate-200/70 dark:border-slate-700/60">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription className="text-slate-600 dark:text-slate-300">
            Sign in to your <span className="font-semibold text-blue-600 dark:text-blue-400">Bazario</span> account
          </CardDescription>
        </CardHeader>
